...
MainWindow::MainWindow() : /*defaultFbo(nullptr), fbo1(nullptr), fbo2(nullptr), bgfbo(nullptr),*/ engine::Window(glm::vec2(800, 600), "Simulator") {
	init();
	// addWidget(new Button_1(this, program_basic));
...
// }
// void MainWindow::removeWidget(Widget *widget) {
// 	if (widget == nullptr) {
// 		return;
// 	}
// 	// else
...
// bool MainWindow::isWidget(Widget *widget) {
...
void MainWindow::init() {
	...
	program_basic.reset(new ShaderProgram(resource.basic_vert(), resource.basic_frag()));
	program_blur7_pingpong_h.reset(new ShaderProgram(resource.blur7_pingpong_h_vert(), resource.blur7_pingpong_h_frag()));
	program_blur7_pingpong_v.reset(new ShaderProgram(resource.blur7_pingpong_v_vert(), resource.blur7_pingpong_v_frag()));
	program_blurN_pingpong_h.reset(new ShaderProgram(resource.blurN_pingpong_h_vert(), resource.blurN_pingpong_h_frag()));
	program_blurN_pingpong_v.reset(new ShaderProgram(resource.blurN_pingpong_v_vert(), resource.blurN_pingpong_v_frag()));
	program_copy_texture.reset(new ShaderProgram(resource.copy_texutre_vert(), resource.copy_texture_frag()));

	// vbo1.reset(new VertexBuffer);
	// vao1.reset(new VertexArray);
	// vao1->bind();
	// vbo1->bind();
	...
	// vbo_blurScreen.reset(new VertexBuffer);
	// vao_blurScreen.reset(new VertexArray);
	// vao_blurScreen->bind();
	// vbo_blurScreen->bind();
...
// void MainWindow::setMatrix(const glm::mat4x4 *m) {
// 	matrix = *m;
// }
...
// void MainWindow::repaint(GLFWwindow *window) {
...
// void MainWindow::updateBackgroundFbo() {
// 		fbo1->bind();
// 		glClear(GL_COLOR_BUFFER_BIT);
// 		glEnable(GL_BLEND);
// 		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
// void MainWindow::DrawCircle(const b2Vec2 &center, float radius, const b2Color &color) {
// 	auto &program = program_basic;
// 	program->bind();
// 	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
// 	vbo1->bind();
...
// void MainWindow::DrawSolidCircle(const b2Vec2 &center, float radius, const b2Vec2 &axis, const b2Color &color) {
// 	auto &program = program_basic;
// 	program->bind();
// 	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
// 	vbo1->bind();
...
// void MainWindow::DrawPoint(const b2Vec2 &p, float size, const b2Color &color) {
// 	DrawSolidCircle(p, size * matrix[3][3], b2Vec2(0, 0), color);
// }

glm::vec2 MainWindow::getMousePos() {
	double xpos, ypos;
