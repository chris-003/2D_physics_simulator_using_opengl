...
namespace engine {
Timer::Timer(const duration_type &duration, BasicTimerServer &timerServer)
    : timer(timerServer.io_service(), duration), timerserver(timerServer) {
    m_duration = duration;
    running    = false;
}
    ...
    if (!e.failed()) {
        ...
        if (which->running) {
            ...
            which->timer.async_wait(
                boost::bind(handler, boost::placeholders::_1, which));
        }
    }
}
} // namespace engine